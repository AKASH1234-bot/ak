...
class Media(Document):
    ...
    request_count = fields.IntField(default=0)   # ← NEW: tracks how many times file was requested
    ...
    async def increment_request_count(cls, file_id: str):
        """Increment request count for trending feature."""
        try:
...
async def ensure_indexes():
    """Create required indexes including request_count for trending."""
    await mydb[COLLECTION_NAME].create_index([('file_name', 'text')])
    await mydb[COLLECTION_NAME].create_index([('request_count', -1)])  # ← NEW: for trending queries
